...
{
	//Check that the file is empty, if not, destroy
	if (dynamicFile) {
		// File exist, delete
		cout << "Overwriting file..." << endl;
		delete[] dynamicFile;
	}
	dynamicFile = new byte[theLength + 1];
	dynamicFile[theLength] = '\0';
	memcpy(dynamicFile, theData, theLength);
	dynamicSize = theLength;
...
byte *FileSystem::readFile(char *path,char *name,udword& theLength)
{
	...
	if (!strcmp(name, "dynamic.htm")) {
		theLength = dynamicSize;
		return dynamicFile;
	}
