...
LLCInPacket::decode()
{
  ...
  if (myDestinationAddress == broadcast || myDestinationAddress == Ethernet::instance().myAddress())
  {
    //cout << "Broadcast" << endl;
    //cout << "LLCDECODE" << endl;
	...
	if(myTypeLen == 0x806){
		// ARP packet
		//cout << "Fick arp" << endl;
		ARPInPacket arpPack(myData, myLength, this);
		arpPack.decode();
  }

	else if (myTypeLen == 0x800) {
		// IP packet
		IPInPacket ipPack(myData, myLength, this);
		ipPack.decode();
	}
